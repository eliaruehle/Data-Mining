...
class Evaluate_Metrics:
    ...
    def write_results_into_file(self, sorted_f_strings: list[str]) -> None:
        ...
        with open(
            "./project_code/dataset_metafeatures/results/unsere_ergebnisse.txt", "w+"
        ) as file:
...
def main():
    ...
    results = pd.read_csv("./project_code/dataset_metafeatures/cosine_sim_results.csv")
    ...
    filtered_results.to_csv(
        "./project_code/dataset_metafeatures/results/master_ergebnisse.txt",
        sep=",",
        index=False,
    )
